...
function openCl(event, clName){
    var i;
    var btn_size;
    var tabcontent;
     btn_size=document.getElementsByClassName("btn_size");
     tabcontent=document.getElementsByClassName("tab-content");
    for(i=0;i<=btn_size.length;i++){
        btn_size[i].className = btn_size[i].className.replace(" active","");
    }
    for(i=0;i<=tabcontent.length;i++){
        tabcontent[i].style.display="none";
    }
    event.currentTarget.className +=" active";
    document.getElementById(clName).style.display="block";
}
